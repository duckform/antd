...
export const isComposeableAdditionalProperties = (
  x: any,
): x is Record<string, any> => {
  return typeof x === "object";
};

export const toJSONSchema = (json: any): JSONSchemaLite => {
  ...
  // const schema = JSON.parse(resp.lines.join(""));
  // return [schema.definitions.Root, schema.definitions];

  // const jsonInput = jsonInputForTargetLanguage("javascript");
  ...
  // return quicktype({ inputData, lang: "schema" }).then((resp) => {
  //   const schema = JSON.parse(resp.lines.join(""));
  //   return [schema.definitions.Root, schema.definitions];
  // });
};
...
export const resolveRefs = (
  ...
  if (isRef(def)) {
    ...
    const properties = def.properties
      ...
      : {};
    const additionalProperties = isComposeableAdditionalProperties(
